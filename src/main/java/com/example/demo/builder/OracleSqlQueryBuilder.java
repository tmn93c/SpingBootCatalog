...
import com.example.demo.filter.TextColumnFilter;
...
public class OracleSqlQueryBuilder {
    ...
    public String totalSql(EnterpriseGetRowsRequest request,String tableName) {
        this.valueColumns = request.getValueCols();
        this.pivotColumns = request.getPivotCols();
        this.groupKeys = request.getGroupKeys();
        this.rowGroupCols = request.getRowGroupCols();
        ...
        this.startRow = request.getStartRow();
        this.endRow = request.getEndRow();
        return "SELECT count(*)" + fromSql(tableName) + whereSql()+";";
    }
    ...
    private String limitSql() {
        // return " OFFSET " + startRow + " ROWS FETCH NEXT " + (endRow - startRow + 1) + " ROWS ONLY";  Oracle
        ...
        return (String columnName, TextColumnFilter filter) -> {
            ...
            switch(filerType) {
                case "equals":
                    ...
                    break;
                case "contains":
                  sql += columnName + " Like '%" + filterValue +"%'";
                  break;
                case "notcontains":
                    sql += columnName + " Not Like '%" + filterValue +"%'";
                  break;
                default:
                  // code block
              }
